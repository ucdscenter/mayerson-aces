...
class TwitterTool():
	...
	def _add_node(self, tweet):
		...
		if d_id in self.nodesdict and not self._is_dummy_node(d_id):
			...
			if tweet["id"] not in self.nodesdict[d_id].tweet_ids:
				...
				self.nodesdict[d_id].retweet_count = self.nodesdict[d_id].retweet_count + tweet["retweet_count"] if "retweet_count" in tweet else self.nodesdict[d_id].retweet_count
		else:
			rtcount = 0
			flcount = 0
			...
			if "followers_count" in tweet["user"]:
				flcount = tweet["user"]["followers_count"]
			self.nodesdict[d_id] = Gnode([], 1, d_id, flcount, rtcount, tweet["hashtags"], tweet["text"], tweet["id"], tweet["user"]["screen_name"],group=self._handlegroup(tweet["user"]["screen_name"]))
			if "quoted_status" in tweet:
				...
				for mention in tweet["user_mentions"][1:]:
					self._add_link(d_id, mention["id"], "mention")
					if mention["id"] not in self.nodesdict:
						self.nodesdict[mention["id"]] = Gnode([], 0, mention["id"], 0, 0, [], "", 0, mention["screen_name"], dummy=True, group=self._handlegroup(mention["screen_name"]))
			else:
	...
	def graph_format(self, readpath, writefile, startday=None, endday=None, usergroups=[[]]):
		...
		print(self.tweet_graph)
		graph_metrics = networkx.node_link_graph(self.tweet_graph, directed=True)
		#print(graph_metrics)
